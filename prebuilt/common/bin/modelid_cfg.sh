...
cat /proc/cmdline|grep androidboot.mid=PC1010000
if [ $? = 0 ];
    then
       rm_kineto=n
fi

cat /proc/cmdline|grep androidboot.mid=PB9910000
if [ $? = 0 ];
    then
       rm_kineto=n
fi

cat /proc/cmdline|grep androidboot.mid=PD1510000
if [ $? = 0 ];
    then
       rm_kineto=n
fi

cat /proc/cmdline|grep androidboot.mid=PB6510000
if [ $? = 0 ];
    then
       rm_kineto=n
fi
...
cat /proc/cmdline | grep spade
if [ $? = 0 ];
   then 
cat /proc/cmdline | grep androidboot.mid=PD9812000
      if [ $? = 0 ];
         then
